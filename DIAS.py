...
@ray.remote
def calculate_DIAS(IRC_idx, xyzString:str, fragments_params: list | dict,  model:str="b973c", clear_log=True):
    # DIAS 함수를 호출하여 계산을 수행합니다.
    result = DIAS(xyzString, model=model, fragments_params=fragments_params, clear_log=clear_log)
    return IRC_idx, result
'''
#ray.init()
...
##############
    추가
##############
def trajDIAS_parallel(trajFile:str, fragments_params: list | dict, model:str="b973c", resultSavePath:str="./aimDIAS_result.json", save_kws:dict={}):
    trajDIASresult = dict()
    trajNum = len(read_traj(trajFile))
    ...
    for IRC_idx, xyzString in enumerate(read_traj(trajFile, returnString=True)):
        progress_bar(trajNum, IRC_idx)
        # 병렬로 실행할 함수를 호출합니다.
        futures.append(calculate_DIAS.remote(IRC_idx, xyzString, model, fragments_params))
    
    # 결과를 수집합니다.
    for future in ray.get(futures):
        IRC_idx, result = future
        trajDIASresult[IRC_idx] = result
...
def trajDIAS_parallel(trajFile:str, fragments_params: list | dict, model:str="b973c", resultSavePath:str="./aimDIAS_result.json", save_kws:dict={}):
    ...
    for IRC_idx, xyzString in enumerate(read_traj(trajFile, returnString=True)):
        progress_bar(trajNum, IRC_idx)
        # 병렬로 실행할 함수를 호출합니다.
        futures.append(calculate_DIAS.remote(IRC_idx, xyzString=xyzString, model=model, fragments_params = fragments_params, clear_log=True))
    # 결과를 수집합니다.
    for future in ray.get(futures):
        IRC_idx, result = future
        trajDIASresult[IRC_idx] = result
...
def aimDIAS_run(trajFile:str, fragments_params: list | dict, mode:str="calculation", DIAresultPath:str="./aimDIAS_result.json", axis_type:str="irc", geo_param:dict | None =None, model:str="b973c", unit:str="kcal/mol", include_fragments=False, relative_idx:None|str|int=0, plot_highlights:dict={"marker": False, "linestyle": True}, resultSavePath:str="./aimDIAS_result.json", horizontal_line:bool=True, save_kws:dict={})->None:
  ...
  if mode == "calculation":
    ...
    # run DIAS
    DIASresult = trajDIAS_parallel(trajFile=trajFile, fragments_params=fragments_params, model=model, resultSavePath=resultSavePath, save_kws=save_kws)
